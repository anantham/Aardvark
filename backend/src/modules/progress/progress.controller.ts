...
export class ProgressController {
  ...
  @Post('start')
  @ApiOperation({ summary: 'Start reading a story or resume existing progress' })
  ...
  async getProgress(@Param('storyId') storyId: string, @Request() req: any) {
    const progress = await this.progressService.getProgress(req.user.id, storyId);
    if (!progress) {
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({ summary: 'Navigate to a previously visited segment (backtrack)' })
  @ApiParam({ name: 'storyId', description: 'Story ID' })
  @ApiParam({ name: 'segmentId', description: 'Target segment ID' })
  @ApiResponse({ status: 200, description: 'Navigation successful' })
  @ApiResponse({ status: 400, description: 'Cannot navigate to unvisited segment' })
  async navigateToSegment(
    @Param('storyId') storyId: string,
    @Param('segmentId') segmentId: string,
    @Request() req: any,
  ) {
    return this.progressService.navigateToSegment(req.user.id, storyId, segmentId);
  }
  ...
  async updateBookmark(
    ...
    return this.progressService.updateBookmark(req.user.id, storyId, segmentId, dto);
  }
