...
export class MessagingController {
  ...
  async getConversations(@Query() query: ConversationQueryDto, @Request() req: any) {
  ...
  async getConversationMessages(
    ...
    return this.messagingService.getMessages(req.user.id, conversationId, query);
  }
  ...
  @ApiResponse({ status: 404, description: 'Conversation not found' })
  @ApiResponse({ status: 403, description: 'Not authorized' })
  async archiveConversation(@Param('id') conversationId: string, @Request() req: any) {
    await this.messagingService.archiveConversation(req.user.id, conversationId);
